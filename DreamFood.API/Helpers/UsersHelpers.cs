namespace DreamFood.API.Helpers
{
    using Common.Models;
    using Domain.Models;
    ...
    public class UsersHelper : IDisposable
    {
        ...
        public static Response CreateUserASP(UserRequest userRequest)
        {
            try
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (!string.IsNullOrEmpty(userRequest.TypeRestaurant))
                    {
                        userManager.AddClaim(newUserASP.Id, new System.Security.Claims.Claim(ClaimTypes.Actor, userRequest.TypeRestaurant));
                    }

                    if (!string.IsNullOrEmpty(userRequest.DistanceRestaurant))
                    {
                        userManager.AddClaim(newUserASP.Id, new System.Security.Claims.Claim(ClaimTypes.PostalCode, userRequest.DistanceRestaurant));
                    }

                    if (!string.IsNullOrEmpty(userRequest.WordKey))
                    {
                        userManager.AddClaim(newUserASP.Id, new System.Security.Claims.Claim(ClaimTypes.UserData, userRequest.WordKey));
                    }
        ...
        public static async Task PasswordRecovery(string email)
        {
            ...
            {
                var subject = "DreamFood App - Recuperación de contraseña";
                var body = string.Format(@"
                    <h1>DreamFood App - Recuperación de contraseña</h1>
